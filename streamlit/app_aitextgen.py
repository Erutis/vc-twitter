...
#this file is for running locally

import streamlit as st
from aitextgen import aitextgen
import time

st.set_page_config(page_title='TLaaS')
st.title("TLaaS")
...
prompt = st.text_input(label='I want my sentence to start with...')


def text_gen():
    ai = aitextgen(model_folder="/Users/kei/Downloads/checkpoint/run1.1", to_gpu=False) 
    st.session_state['text'] = ai.generate_one(max_length=100, min_length=20, temperature=.9, prompt=prompt)
...
try:
    ...
    st.write('This will change to generated text once you press the button.')
